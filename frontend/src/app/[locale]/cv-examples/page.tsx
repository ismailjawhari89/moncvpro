...
import { Search, Filter, ArrowRight, Briefcase, Layout, Zap } from 'lucide-react';
...
export default function CVExamplesPage() {
    ...
    const industries = ['All', ...Array.from(new Set(examples.map(ex => ex.industry)))];
    const levels = ['All', ...Array.from(new Set(examples.map(ex => ex.level)))];
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
                    ...
                    <div className="max-w-2xl mx-auto relative">
                        <input
                            ...
                            className="w-full px-6 py-4 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white placeholder-blue-200 focus:outline-none focus:ring-2 focus:ring-white/50 pl-12 shadow-xl"
                        />
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                            {filteredExamples.map((example) => (
                                <Link
                                    ...
                                    key={example.id}
                                    className="group bg-white rounded-xl shadow-sm hover:shadow-xl border border-gray-200 overflow-hidden transition-all duration-300 hover:-translate-y-1 flex flex-col"
                                >
                                    {/* Thumbnail */}
